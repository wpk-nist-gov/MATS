    ...
    for n1 in range(1, Nmax + 2, 2):
        ...
        for n2 in range(abs(n1 - lso), n1 + lso + 2, 2):
            ...
            for iw in range(len(omegas)):
                kp = np.sqrt(
                    2.0 * mumass * (kboltz * Temp + abs(omegas[iw] * cm1 - dE))
                )
                mga = (ahard * (k - kp) / 2) ** 2
                if abs(mga) < 260:
                    mgf = mpmath.meijerg(
                        [[0], []], [[0, 1.5, 2], []], abs(mga) + 1e-300
                    ) / (k * kp)
                else:
                    mgf = 0.0
                mgf = (
                    mgf
                    * (1.0 + np.exp((omegas[iw] * cm1 - dE) / (kboltz * Temp)))
                    / (2.0 * np.cosh((omegas[iw] * cm1 - dE) / (kboltz * Temp)))
                )
...
def Karman_CIA_Model(
    ...
    if SO_scalar == 0:
        ...
        VGSO = SO_scalar * SO(
            omegas, ahard=SO_ahard, lso=int(SO_l), Nmax=Nmax, Temp=temperature + 273.15
        )
    ...
    f = interpolate.interp1d(
        sim_wave, CIA_model_sim, bounds_error=False, fill_value="extrapolate"
    )
