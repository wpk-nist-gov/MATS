...
def HTP_from_DF_select(
    ...
    for molec in linelist["molec_id"].unique():
        for iso in linelist["local_iso_id"].unique():
            ...
            linelist.loc[
                ...
                molec, iso, isotope_list=isotope_list
            )  # * 1.66053873e-27 * 1000 #cmassmol and kg conversion
    ...
    linelist["LineIntensity"] = (
        linelist["sw"]
        * linelist["SigmaTref"]
        / linelist["SigmaT"]
        * (
            np.exp(-CONSTANTS["c2"] * linelist["elower"] / T)
            * (1 - np.exp(-CONSTANTS["c2"] * linelist["nu"] / T))
        )
        / (
            np.exp(-CONSTANTS["c2"] * linelist["elower"] / Tref)
            * (1 - np.exp(-CONSTANTS["c2"] * linelist["nu"] / Tref))
        )
    )
    if isotope_list != ISO:
        linelist.loc[
            (linelist["SigmaT"] == 1) & (linelist["SigmaTref"] == 1), "LineIntensity"
        ] = linelist[(linelist["SigmaT"] == 1) & (linelist["SigmaTref"] == 1)][
            "sw"
        ].values
...
def HTP_wBeta_from_DF_select(
    ...
    if Diluent is None:
        ...
        if diluent == "air":
            ...
            # Diluent = {diluent: {"composition": 1, "m": 0}}
            # print(
            #     "THIS IS GOING TO BREAK WITH A DIVISION ERROR.  GO BACK AND USE DILUENT FORMALISM FOR THE SPECTRUM DEFINITION"
            # )
            raise ValueError(
                f"diluent {diluent}. THIS IS GOING TO BREAK WITH A DIVISION ERROR.  GO BACK AND USE DILUENT FORMALISM FOR THE SPECTRUM DEFINITION"
            )
    ...
    for molec in linelist["molec_id"].unique():
        for iso in linelist["local_iso_id"].unique():
            ...
            if (len(Diluent) == 1) & ("self" in Diluent):
                Diluent["self"]["mp"] = molecular_mass(
                    molec, iso, isotope_list=isotope_list
                )
            if (not natural_abundance) and abundance_ratio_MI != {}:
                linelist.loc[
    ...
    if isotope_list != ISO:
        linelist.loc[
            (linelist["SigmaT" == 1]) & (linelist["SigmaTref" == 1])
        ] = linelist[(linelist["SigmaT" == 1]) & (linelist["SigmaTref" == 1])][
            "sw"
        ].values
    ...
    linelist["B"] = (
        1.9595
        - 0.1258 * linelist["alpha"].values
        + 0.0056 * linelist["alpha"].values ** 2
        + 0.0050 * linelist["alpha"].values ** 3
    )
    linelist["C"] = (
        -0.0546
        + 0.0672 * linelist["alpha"].values
        - 0.0125 * linelist["alpha"].values ** 2
        + 0.0003 * linelist["alpha"].values ** 3
    )
    linelist["D"] = 0.9466 - 0.1585 * np.exp(-0.4510 * linelist["alpha"].values)
    linelist["Beta"] = (
        linelist["A"].values
        * np.tanh(
            linelist["B"].values * np.log10(linelist["Chi"].values)
            + linelist["C"].values
        )
        + linelist["D"].values
    )
...
class Fit_DataSet:
        ...
        if self.CIA_calculate & (self.CIAparam_list is not None):
            if self.CIA_model is not None:
                ...
                for CIA_param in list(self.CIAparam_list):
                    if (
                        ("_vary" not in CIA_param)
                        and ("_err" not in CIA_param)
                        and ("CIA Pair" not in CIA_param)
                    ):
                ...
                for index in self.CIAparam_list.index.values:
                    ...
                    for CIA_param in CIA_parameters:
                        params.add(
                            CIA_param + "_" + CIA_pair,
                            self.CIAparam_list.loc[index][CIA_param],
    ...
    def constrained_baseline(
        ...
        etalon_period_segment_constrained=True,
    ...
    def simulation_model(
        ...
        for param in list(params.valuesdict().keys()):
            if (
                ...
                ("EXCH_scalar" in param)
                or ("EXCH_gamma" in param)
                or ("EXCH_l" in param)
                or ("SO_scalar" in param)
                or ("SO_ahard" in param)
                or ("SO_ahard" in param)
                or ("SO_l" in param)
                or ("bandcenter" in param)
                or ("Nmax" in param)
            ):
        ...
        for spectrum in self.dataset.spectra:
            ...
            for segment in list(set(spectrum.segments)):
                ...
                for molecule in spectrum.molefraction:
                    if (
                        "molefraction_" + self.dataset.isotope_list[(molecule, 1)][4]
                    ) + "_" + str(spectrum_number) + "_" + str(
                        segment
                    ) in baseline_params:
                        fit_molefraction[molecule] = np.float(
                            params[
                                (
                                    "molefraction_"
                                    + self.dataset.isotope_list[(molecule, 1)][4]
                                )
                                + "_"
                                + str(spectrum_number)
                                + "_"
                ...
                if self.beta_formalism:
                    fit_nu, fit_coef = HTP_wBeta_from_DF_select(
                        linelist_for_sim,
                        wavenumbers,
                    ...
                    for CIA_pair in CIA_pairs:
                        for molecule in self.dataset.molecule_list:
                            ...
                            for broadener in self.dataset.broadener_list:
                                try:
                                    broadener_composition = spectrum.Diluent[broadener][
                                        "composition"
                                    ]
                                except Exception:
                            ...
                            if (molecule_name in CIA_pair) and (broadener in CIA_pair):
                                CIA += broadener_composition * Karman_CIA_Model(
                                    wavenumbers,
                                    p * 760,
                                    T - 273.15,
                                    wave_step=self.CIA_wavestep,
                                    EXCH_scalar=params["EXCH_scalar" + "_" + CIA_pair],
                                    EXCH_gamma=params["EXCH_gamma" + "_" + CIA_pair],
                                    EXCH_l=params["EXCH_l" + "_" + CIA_pair],
                                    SO_scalar=params["SO_scalar" + "_" + CIA_pair],
                                    SO_ahard=params["SO_ahard" + "_" + CIA_pair],
                                    SO_l=params["SO_l" + "_" + CIA_pair],
                                    bandcenter=params["bandcenter" + "_" + CIA_pair],
                                    Nmax=params["Nmax" + "_" + CIA_pair],
                                )
                else:
                    CIA = spectrum.cia[
                ...
                for param in baseline_params:
                    if ("etalon" in param) and (
                        ...
                        if param == "etalon_" + str(etalon_num) + "_period_" + str(
                            spectrum_number
                        ) + "_" + str(
                            segment
                        ):  # ('period' in param) and (str(etalon_num) in param):
                            fit_etalon_parameters[etalon_num]["period"] = np.float(
                                params[param]
                            )
                ...
                for i in range(1, len(spectrum.etalons) + 1):
                    etalons += etalon(
                        ...
                        fit_etalon_parameters[i]["period"],
                        fit_etalon_parameters[i]["phase"],
                    )
                segment_alpha = baseline + etalons + fit_coef + CIA
                # ILS_Function
                if spectrum.ILS_function is not None:
                    if (
                        self.dataset.ILS_function_dict[spectrum.ILS_function.__name__]
                        == 1
                    ):
                        spec_seg_ILS_resolution = np.float(
                            params[
                                spectrum.ILS_function.__name__
                                + "_res_0_"
                                + str(spectrum_number)
                                + "_"
                                + str(segment)
                        ...
                        for res_param in range(
                            0,
                            self.dataset.ILS_function_dict[
                                spectrum.ILS_function.__name__
                            ],
                        ):
                            spec_seg_ILS_resolution += np.float(
                    ...
                    wavenumbers, segment_alpha, i1, i2m, slit = convolveSpectrumSame(
                        wavenumbers,
                        segment_alpha,
                        SlitFunction=spectrum.ILS_function,
                        Resolution=spec_seg_ILS_resolution,
                        AF_wing=spectrum.ILS_wing,
                    )
                simulated_spectra[
                    np.min(indices_segments[segment]) : np.max(
                        indices_segments[segment]
                ...
                if self.weight_spectra:
                    ...
                    residuals[
                        ...
                        + 1
                    ] = ((segment_alpha) - alpha_segments[segment]) * weights
                else:
                    residuals[
                        np.min(indices_segments[segment]) : np.max(
                            indices_segments[segment]
                        ...
                        + 1
                    ] = (segment_alpha) - alpha_segments[segment]
    ...
    def update_params(
        ...
        for key, par in result.params.items():
            if "__lnsigma" in par.name:
                ...
                indices = [
                    m.start()
                    for m in re.finditer("_", par.name[par.name.find("_res_") + 5 :])
                ]
                # spec_num = par.name[par.name.find("_res_") + 5 :][
                #     indices[0] + 1 : indices[1]
                # ]
                # seg_num = par.name[par.name.find("_res_") + 5 :][indices[1] + 1 :]
                parameter = (
                    par.name[: par.name.find("_res_")]
                    + "_res_"
                    + par.name[par.name.find("_res_") + 5 :][: indices[0]]
                )
                self.baseline_list.loc[
                    (self.baseline_list["Segment Number"] == segment)
                    & (self.baseline_list["Spectrum Number"] == spectrum),
                ...
                ("EXCH_scalar" in par.name)
                or ("EXCH_gamma" in par.name)
                or ("EXCH_l" in par.name)
                or ("SO_scalar" in par.name)
                or ("SO_ahard" in par.name)
                or ("SO_l" in par.name)
            ):
                if self.CIA_calculate and self.CIA_model == "Karman":
                    indices = [m.start() for m in re.finditer("_", par.name)]
                    ...
                    self.CIAparam_list.loc[
                        (self.CIAparam_list["CIA Pair"] == CIA_pair), parameter
                    ] = par.value
                    if par.vary:
                        self.CIAparam_list.loc[
                            (self.CIAparam_list["CIA Pair"] == CIA_pair),
                            parameter + "_err",
                        ] = par.stderr
        ...
        if CIA_linelist_update_file is not None:
                    ...
                    for CIA_pair in CIA_pairs:
                        for molecule in self.dataset.molecule_list:
                            ...
                            for broadener in self.dataset.broadener_list:
                                ...
                                if broadener == "self":
                                    broadener = molecule_name
                                if (molecule_name in CIA_pair) and (
                                    broadener in CIA_pair
                                ):
                                    CIA_select = self.CIAparam_list[
                                        self.CIAparam_list["CIA Pair"] == CIA_pair
                                    ]
                                    CIA += broadener_composition * Karman_CIA_Model(
                                        spectrum.wavenumber,
                                        spectrum.get_pressure_torr(),
                                        spectrum.get_temperature_C(),
                                        wave_step=self.CIA_wavestep,
                                        EXCH_scalar=CIA_select["EXCH_scalar"].values[0],
                                        EXCH_gamma=CIA_select["EXCH_gamma"].values[0],
                                        EXCH_l=CIA_select["EXCH_l"].values[0],
                                        SO_scalar=CIA_select["SO_scalar"].values[0],
                                        SO_ahard=CIA_select["SO_ahard"].values[0],
                                        SO_l=CIA_select["SO_l"].values[0],
                                        bandcenter=CIA_select["bandcenter"].values[0],
                                        Nmax=CIA_select["Nmax"].values[0],
                                    )
        ...
        if self.beta_formalism:
            ...
            if (
                (
                    sum(
                        ...
                        & ("_err" not in param)
                        & ("_vary" not in param)
                        for param in beta_summary_list
                    )
                )
            ) > 1:
                nu_constrain = False
            ...
            for molec in beta_summary_list["molec_id"].unique():
                for iso in beta_summary_list["local_iso_id"].unique():
                    beta_summary_list.loc[
                        (beta_summary_list["molec_id"] == molec)
                        & (beta_summary_list["local_iso_id"] == iso),
                        "m",
                    ] = molecular_mass(
                        molec,
                        iso,
            ...
            for spectrum in self.dataset.spectra:
                ...
                for diluent in spectrum.Diluent:
                    mp += (
                        spectrum.Diluent[diluent]["composition"]
                        * spectrum.Diluent[diluent]["m"]
                    )

                for segment in list(set(spectrum.segments)):
                    p = self.baseline_list[
                        (
                            self.baseline_list["Spectrum Number"]
                            == spectrum.spectrum_number
                        )
                        & (self.baseline_list["Segment Number"] == segment)
                    ]["Pressure"].values[0]
                    T = self.baseline_list[
                        (
                            self.baseline_list["Spectrum Number"]
                            == spectrum.spectrum_number
                        )
                    ...
                    if nu_constrain:
                        GammaD = (
                            np.sqrt(
                                ...
                                * T
                                * np.log(2)
                                / (beta_summary_list["m"].values)
                            )
                            * beta_summary_list["nu"]
                            / CONSTANTS["c"]
                        )  # change with nu
                    else:
                        GammaD = (
                            ...
                            * beta_summary_list[
                                "nu" + "_" + str(spectrum.spectrum_number)
                            ]
                            / CONSTANTS["c"]
                        )  # change with nu
                    nuVC = len(GammaD) * [0]
                    for diluent in spectrum.Diluent:
                        ...
                        if nuVC_constrain:
                            nuVC += abun * (
                                beta_summary_list["nuVC_%s" % diluent]
                                * (p / 1)
                                * (
                                    (296 / T)
                                    ** (beta_summary_list["n_nuVC_%s" % diluent])
                                )
                            )
                        else:
                            nuVC += abun * (
                                ...
                                * (
                                    ...
                                    ** (
                                        beta_summary_list[
                                            "n_nuVC_%s_%s"
                                            % (diluent, str(spectrum.spectrum_number))
                                        ]
                                    )
                                )
                            )
                    Chi = nuVC / GammaD
                    A = 0.0534 + 0.1585 * np.exp(
                        -0.451 * beta_summary_list["alpha"].values
                    )
                    B = (
                        1.9595
                        - 0.1258 * beta_summary_list["alpha"].values
                        + 0.0056 * beta_summary_list["alpha"].values ** 2
                        + 0.0050 * beta_summary_list["alpha"].values ** 3
                    )
                    C = (
                        -0.0546
                        + 0.0672 * beta_summary_list["alpha"].values
                        - 0.0125 * beta_summary_list["alpha"].values ** 2
                        + 0.0003 * beta_summary_list["alpha"].values ** 3
                    )
                    D = 0.9466 - 0.1585 * np.exp(
                        -0.4510 * beta_summary_list["alpha"].values
                    )
                    beta_summary_list["beta"] = A * np.tanh(B * np.log10(Chi) + C) + D
                    beta_summary_list.loc[
                        (beta_summary_list["nu"] >= wave_min)
                        & (beta_summary_list["nu"] <= wave_max),
                        "Beta_" + str(spectrum.spectrum_number),
                    ] = beta_summary_list[
                        (beta_summary_list["nu"] >= wave_min)
                        & (beta_summary_list["nu"] <= wave_max)
                    ][
                    ...
                    beta_summary_list.loc[
                        (beta_summary_list["nu"] >= wave_min)
                        & (beta_summary_list["nu"] <= wave_max),
                        "Chi_" + str(spectrum.spectrum_number),
                    ] = beta_summary_list[
                        (beta_summary_list["nu"] >= wave_min)
                        & (beta_summary_list["nu"] <= wave_max)
                    ][
