...
class Spectrum:
    ...
    def __init__(
        ...
        if Diluent is None:
            # TODO: fix this
            # if Diluent was not set as the dictionary of various broadeners,
            # then define dictionary with all of the broadening contribution coming
            # from the diluent broadener
            if self.diluent == "air":
                ...
                print(
                    'If using the HTP_wBeta_from_DF_select then you need to go back and use the Diluent{diluent:{"composition": 1, "m": mass}} format'
                )
                self.Diluent = {self.diluent: {"composition": 1, "m": 0}}
    ...
    def plot_model_residuals(self):
        ...
        ax0.set_ylabel(r"$\alpha (\frac{ppm}{cm})$")
...
def simulate_spectrum(
    ...
    if Diluent is None:
        if diluent == "air":
            ...
            Diluent = {diluent: {"composition": 1, "m": 0}}
            print(
                "THIS IS GOING TO BREAK WITH A DIVISION ERROR IF YOU USE THE BETA VERSION"
            )
    ...
    for seg in range(0, num_segments):
        ...
        if beta_formalism:
            waves, alpha = HTP_wBeta_from_DF_select(
                parameter_linelist,
                waves,
                wing_cutoff,
                wing_wavenumbers,
    ...
    if ILS_function is not None:
        wavenumbers, alpha_array, i1, i2m, slit = convolveSpectrumSame(
            wavenumbers,
            alpha_array,
            SlitFunction=ILS_function,
            Resolution=ILS_resolution,
            AF_wing=ILS_wing,
        )
    ...
    spectrum["Noise (%)"] = (
        100
        * (alpha_noise - (alpha_array + baseline + etalon_model))
        / np.max(alpha_noise)
    )
