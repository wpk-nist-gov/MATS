...
class Dataset:
    ...
    def __init__(
        self, spectra, dataset_name, param_linelist, baseline_order=1, CIA_model=None
    ):
    ...
    def check_iso_list(self):
        ...
        molecules_isotopes_in_paramlist = (
            self.param_linelist.groupby(["molec_id", "local_iso_id"])
            .size()
            .reset_index()
        ...
        for i in molecules_isotopes_in_paramlist.index:
            molecule = molecules_isotopes_in_paramlist[
                molecules_isotopes_in_paramlist.index == i
            ]["molec_id"].values[0]
            isotope = molecules_isotopes_in_paramlist[
                molecules_isotopes_in_paramlist.index == i
            ]["local_iso_id"].values[0]
        ...
        for spectrum in self.spectra:
            if spectrum.isotope_list != ISO:
                if spectrum.isotope_list not in other_isotope_list:
                    ...
                    isolist_molecules_isotopes = isotope_list_molecules_isotopes(
                        isotope_list=spectrum.isotope_list
                    )
                    for molecule in self.molecule_list:
                        if molecule not in isolist_molecules_isotopes:
                            if molecule in molecules_isotopes_in_paramlist_dict:
                                missing_molecules[
                                    molecule
                                ] = molecules_isotopes_in_paramlist_dict[molecule]
                            else:
                                missing_molecules[molecule] = ["all isotopes missing"]
                        else:
                            for isotope in molecules_isotopes_in_paramlist_dict[
                                molecule
                            ]:
        ...
        if len(missing_molecules) == 0 and len(other_isotope_list) == 1:
            for spectrum in self.spectra:
                if spectrum.isotope_list != other_isotope_list[0]:
                    ...
                    print(
                        "Best practice is to use the same isotope list for all spectra.  All molecules were found in the non-HITRAN isotope list, so this has been set as the isotope list for all molecules."
                    )
            return other_isotope_list[0]
        elif len(other_isotope_list) > 1 and (len(missing_molecules) == 0):
            print(
                "WARNING:  Use the same isotope list for all spectra to ensure continuity in the dataset"
            )
            return ISO
        elif len(missing_molecules) != 0:
            print(missing_molecules)
            print(
                "WARNING:  Use the same isotope list for all spectra to ensure continuity in the dataset and make sure all Molecules and isotopes are in that isotope list."
            )
            return ISO
        else:
            return ISO
        ...
        for spectrum in self.spectra:
            if spectrum.ILS_function is not None:
                if spectrum.ILS_function.__name__ not in dataset_ILS_function_dict:
                    if (type(spectrum.ILS_resolution) == float) or (
                        type(spectrum.ILS_resolution) == int
                    ):
                        dataset_ILS_function_dict[spectrum.ILS_function.__name__] = 1
                    else:
                        dataset_ILS_function_dict[spectrum.ILS_function.__name__] = len(
                            spectrum.ILS_resolution
                        )
    ...
    def generate_baseline_paramlist(self):
        ...
        for spectrum in self.spectra:
            for segment in list(set(spectrum.segments)):
                ...
                for molecule in spectrum.molefraction:
                    line[
                        "molefraction_" + (self.isotope_list[(molecule, 1)][4])
                ...
                for etalon_name in spectrum.etalons:
                    ...
                    line["etalon_" + str(etalon_name) + "_period"] = spectrum.etalons[
                        etalon_name
                    ][1]
                ...
                if self.ILS_function_dict != {}:
                    for ILS_function in self.ILS_function_dict:
                        for res_param in range(0, self.ILS_function_dict[ILS_function]):
                            if (spectrum.ILS_function is None) or (
                                spectrum.ILS_function.__name__ != ILS_function
                            ):
                                line[ILS_function + "_res_" + str(res_param)] = 0
                            elif (type(spectrum.ILS_resolution) == float) or (
                                type(spectrum.ILS_resolution) == int
                            ):
                                line[
                                    ILS_function + "_res_" + str(res_param)
                                ] = spectrum.ILS_resolution
                            else:
                                line[
                                    ILS_function + "_res_" + str(res_param)
                                ] = spectrum.ILS_resolution[res_param]
        ...
        baseline_paramlist.to_csv(
            self.dataset_name + "_baseline_paramlist.csv", index=True
        )
            ...
            for molecule in self.molecule_list:
                ...
                for broadener in self.broadener_list:
                    if broadener == "self":
                        ...
                        if (molecule_name + "_" + broadener not in CIA_list) & (
                            broadener + "_" + molecule_name not in CIA_list
                        ):
