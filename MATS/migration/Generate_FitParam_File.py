...
class Generate_FitParam_File:
    ...
    def generate_fit_param_linelist_from_linelist(
        self,
        vary_nu={7: {1: True, 2: False, 3: False}, 1: {1: False}},
        vary_sw={7: {1: True, 2: False, 3: False}},
        vary_gamma0={7: {1: True, 2: False, 3: False}, 1: {1: False}},
        vary_n_gamma0={},
        vary_delta0={7: {1: True, 2: False, 3: False}, 1: {1: False}},
        vary_n_delta0={},
        ...
        for param in ordered_list:
            if ("SD_gamma" in param) & ("vary" not in param) & ("err" not in param):
                if (
                    len(
                        param_linelist_df[
                            (param_linelist_df[param] == 0)
                            & (param_linelist_df[param + "_vary"])
                        ]
                    )
                    != 0
                ):
                    print(
                        param,
                        ": floating SD_gamma terms when the initial guess is zero can make it difficult for the solution to converge. Consider setting initial guess to a non-zero value.",
                    )
            if ("SD_delta" in param) & ("vary" not in param) & ("err" not in param):
                if (
                    ...
                    print(
                        param,
                        ": floating SD_delta terms when the initial guess is zero can make it difficult for the solution to converge. Consider setting initial guess to a non-zero value.",
                    )
            if (
                ("nuVC" in param)
                & ("vary" not in param)
                ...
                if (
                    ...
                    print(
                        param,
                        ": floating nuVC terms when the initial guess is zero can make it difficult for the solution to converge. Consider setting initial guess to a non-zero value.  This is generally less of an issue than the case where SD_gamma is floated when the initial guess is zero.",
                    )
    ...
    def generate_fit_baseline_linelist(
        ...
        vary_molefraction={7: True, 1: False},
        vary_xshift=False,
        ...
        for param in parameters:
            ...
            if "phase" in param:
                base_linelist_df.loc[
                    base_linelist_df[param.replace("phase", "period")] != 0,
                    param + "_vary",
                ] = vary_etalon_phase
            if "_res_" in param:
                base_linelist_df.loc[
    ...
    def generate_fit_KarmanCIA_linelist(
        self,
        vary_EXCH_scalar=False,
        vary_EXCH_gamma=False,
        vary_EXCH_l=False,
        vary_SO_scalar=False,
        vary_SO_ahard=False,
        vary_SO_l=False,
        vary_bandcenter=False,
        vary_Nmax=False,
        wave_step=5,
    ):
        ...
        if self.dataset.CIA_model == "Karman":
                    ...
                    for molecule in self.dataset.molecule_list:
                        ...
                        for broadener in self.dataset.broadener_list:
                            ...
                            if (molecule_name in CIA_pair) and (broadener in CIA_pair):
                                CIA_select = CIA_linelist_df[
                                    CIA_linelist_df["CIA Pair"] == CIA_pair
                                ]
            ...
            for param in parameters:
                ...
                if "EXCH_scalar" in param:
                    CIA_linelist_df[param + "_vary"] = len(CIA_linelist_df) * [
                        (vary_EXCH_scalar)
                    ]
            ...
            CIA_linelist_df = CIA_linelist_df[
                [
                    "CIA Pair",
                    "EXCH_scalar",
                    "EXCH_scalar_err",
            ...
            print(
                "Generate_fit_KarmanCIA_linelist only applies to the CIA files generated for the Karman CIA model"
            )
            return None
