...
from ..hapi import ISO, ISO_INDEX

# Constants
CONSTANTS = dict(
    # erg s https://physics.nist.gov/cgi-bin/cuu/Value?h|search_for=h as of 5/21/2020
    h=6.62607015e-27,
    # cm/s # https://physics.nist.gov/cgi-bin/cuu/Value?c|search_for=c as of 5/21/2020
    c=29979245800,
    # erg / K https://physics.nist.gov/cgi-bin/cuu/Value?k as of 5/21/2020
    k=1.380649e-16,
    # mol-1 https://physics.nist.gov/cgi-bin/cuu/Value?na as of 5/21/2020
    Na=6.02214076e23,
    # convert from cpa to atm  https://physics.nist.gov/cgi-bin/cuu/Value?stdatm|search_for=atmosphere as of 5/21/2020
    cpa_atm=(10 * 101325) ** -1,
)
...
def molecular_mass(mol_num, iso_num, isotope_list=None):
    ...
    INPUT PARAMETERS:
        ...
        iso_num: HITRAN isotopologue number
    OUTPUT PARAMETERS:
    ...
    return isotope_list[(mol_num, iso_num)][ISO_INDEX["mass"]]


def isotope_list_molecules_isotopes(isotope_list=None):
...
def add_to_HITRANstyle_isotope_list(
    input_isotope_list=ISO,
    molec_id=100,
    local_iso_id=1,
    global_isotope_id=200,
    iso_name="",
    abundance=1,
    mass=1,
    mol_name="",
    ...
    if (molec_id, local_iso_id) in input_isotope_list:
        print(
            "Already entry with that molec_id and local_iso_id.  This result will write over that entry."
        )
    elif (molec_id, 1) in input_isotope_list:
        mol_name_ = input_isotope_list[(molec_id, 1)][ISO_INDEX["mol_name"]]
        print(
            "This is being added as a new isotope of "
            + mol_name_
            + ".  Proceed if that was the intention."
        )
    global_iso_id_list = []
    ...
    if global_isotope_id in global_iso_id_list:
        print(
            "There is another entry with this global isotope id.  Consider changing the value for consistency"
        )
    output_isotope_list = input_isotope_list.copy()
    output_isotope_list[(molec_id, local_iso_id)] = [
        global_isotope_id,
        iso_name,
        abundance,
        mass,
