...
def HTP_from_DF_select(
    ...
    linelist["LineIntensity"] = (
        ...
        * linelist["SigmaTref"]
        / linelist["SigmaT"]
        * (
            np.exp(-c2 * linelist["elower"] / T)
            * (1 - np.exp(-c2 * linelist["nu"] / T))
        )
        / (
            np.exp(-c2 * linelist["elower"] / Tref)
            * (1 - np.exp(-c2 * linelist["nu"] / Tref))
        )
    )
    if isotope_list != ISO:
        linelist.loc[
    ...
    linelist["GammaD"] = (
        np.sqrt(2 * k * Na * T * np.log(2) / (linelist["m"].values))
        * linelist["nu"]
        / c
    )
            ...
            if (
                ...
                for line_param in linelist_params:
                    ...
                    # linemixing
                        ...
                        and (not line_param in linemix_terms_constrained)
                    ):
                        if (
                            self.linemixing_limit
                            and self.lineparam_list.loc[spec_line][line_param] != 0
